...
// Atalhos de teclado: Alt+1-4 para navegar entre telas
document.addEventListener('keydown', function(e) {
  if (!e.altKey) return;
...
function exportarBackup() {
  ...
  a.href     = url;
  a.download = `SaldoMais-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
  ...
  reader.onload = function(ev) {
    try {
      ...
      if (!data.categorias || !data.orcamentos || !data.lancamentos) {
        notificar('Arquivo inválido — formato não reconhecido');
        return;
      }
